//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIView.h"

@class UIImageView, UILabel;

@interface WAShareContactTableHeaderView : UIView
{
    UILabel *_nameLabel;
    UIImageView *_profilePhotoImageView;
    UIImageView *_checkmarkImageView;
    UIView *_checkmarkBackgroundView;
    _Bool _profilePhotoSharingEnabled;
}

@property(nonatomic) _Bool profilePhotoSharingEnabled; // @synthesize profilePhotoSharingEnabled=_profilePhotoSharingEnabled;
- (void).cxx_destruct;
- (void)photoTapped:(id)arg1;
- (void)configureWithNameSharingUnit:(id)arg1 profilePhotoSharingUnit:(id)arg2;
- (void)layoutSubviews;
- (id)initWithFrame:(struct CGRect)arg1;

@end

